...
- **Animated-property allowlist (the reflow rule).** On **flow words** (`.flow .w`, any `.w`/`.cap` span sharing a line box) NEVER animate `letter-spacing`, `font-size`, `font-weight`, or `filter:blur` — a width change triggers inline-block line-box reflow, so neighbouring words jump rows mid-tween (the "Some → line 2" bug, [references/failure-modes.md](failure-modes.md) § Animation). The isolated **climax / hero `span`** is the one exemption: not inside a multi-word line box, so its sanctioned standalone entrances MAY animate `letterSpacing` / `blur` / `fontVariationSettings` (e.g. monument, tracking, hairrise in `_motion.md`; the glassslab / starfield apex).
